...
function AdminPanel() {
    ...
    const portalData = {
        ...
        'Current Affairs': {
            ...
            // No Subjects or Types here. Just direct upload to categories!
        },
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            await axios.post('https://study-marrow-api.onrender.com/api/verify-admin', {}, { headers: { 'admin-key': passwordInput } });
    ...
    const fetchSubscribers = async (key) => {
        ...
        } catch (err) { console.error("Error fetching subs"); }
    };
    ...
    const handleUpload = async (e) => {
        ...
        // Validation based on Vertical
        if (vertical === 'Job Exam Preparation') return alert("This section is under development!");
        
        if (vertical === 'Current Affairs') {
            if (!category || !title || !link) return alert("Please fill Category, Title, and Link!");
            finalSubject = ''; // Bypass subject
            finalType = 'Current Affairs'; // Set a default type so the database doesn't complain
        } else {
            if (!category || !subject || !resourceType || !title || !link) return alert("Please fill all fields!");
            if (resourceType.includes('Papers') && !board) return alert("⚠️ Please select a Board (CBSE or ASSEB)!");
        }

        const materialData = { vertical, category, subject: finalSubject, resourceType: finalType, title, link, board };

        try {
            await axios.post('https://study-marrow-api.onrender.com/api/upload', materialData, { headers: { 'admin-key': adminKey } });
            alert('✅ Link Added Successfully!');
            setTitle(''); setLink(''); setBoard('');
            fetchMaterials(); 
        } catch (err) { alert('Upload failed: ' + (err.response?.data?.message || "Error")); }
    };

    // --- EDIT, DELETE & REORDER ---
    const handleEdit = async (id, currentTitle) => {
        const newTitle = window.prompt("New name:", currentTitle);
    ...
    const handleMove = async (fileToMove, direction, config) => {
        const currentList = materials.filter(m => 
            ...
            (config.sub === '' ? (m.subject === '' || !m.subject) : m.subject === config.sub)
        );
    ...
    // --- RENDER HELPERS ---
    const getFiles = (vert, cat, sub) => materials.filter(m => 
        (m.vertical === vert || (!m.vertical && vert === 'School Academics')) && 
        m.category === cat && 
        (sub === '' ? (m.subject === '' || !m.subject) : m.subject === sub)
    );

    // --- UI ---
    if (!isAuthenticated) return (
        <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#f1f5f9' }}>
            <form onSubmit={handleLogin} style={{ background: 'white', padding: '40px', borderRadius: '15px', textAlign: 'center', maxWidth: '400px', width:'100%' }}>
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto', fontFamily: 'sans-serif' }}>
            ...
            {/* --- UPLOAD FORM --- */}
            <div style={{ background: '#f8f9fa', padding: '25px', borderRadius: '12px', marginBottom: '30px', border: '1px solid #e2e8f0' }}>
                <h2 style={{ marginTop: 0, color: '#2563eb' }}>Add New Material</h2>
                <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <input type="text" placeholder="Title (e.g., March Week 1 PDF or Chapter 1 Notes)" value={title} onChange={(e) => setTitle(e.target.value)} required style={inputStyle} />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: vertical === 'Current Affairs' ? '1fr' : '1fr 1fr', gap: '15px' }}>
                        ...
                        {/* HIDE SUBJECT IF CURRENT AFFAIRS */}
                    ...
                    {/* HIDE RESOURCE TYPE IF CURRENT AFFAIRS */}
                    {vertical === 'School Academics' && (
                    ...
                    <input type="url" placeholder="Paste Link..." value={link} onChange={(e) => setLink(e.target.value)} required style={inputStyle} />
            ...
            {/* --- LIBRARY OVERVIEW --- */}
            <div>
                <h2 style={{borderBottom: '2px solid #333', paddingBottom: '10px', marginBottom: '20px'}}>📚 Portal Library</h2>
                    ...
                    <div key={vert} style={sectionStyle}>
                        ...
                        {portalData[vert].categories.map(cat => (
                            <div key={cat} style={{ marginTop: '20px' }}>
                                <h3 style={{ color: '#334155', background: '#f8fafc', padding: '10px', borderRadius: '5px' }}>{cat}</h3>
                                
                                {/* DIRECT FILE LIST FOR CURRENT AFFAIRS */}
                                {vert === 'Current Affairs' ? (
                                    <div style={{ marginLeft: '10px', marginBottom: '15px' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {getFiles(vert, cat, '').length === 0 ? <small style={{color:'#94a3b8'}}>No files yet.</small> : 
                                                getFiles(vert, cat, '').map((f, idx, arr) => (
                                    ...
                                    /* FOLDERS FOR SCHOOL ACADEMICS */
                                    portalData[vert].subjects[cat].map(sub => (
                                        <div key={sub} style={{ marginLeft: '20px', borderLeft: '2px solid #cbd5e1', paddingLeft: '15px', marginBottom: '15px' }}>
                                            <h4 style={{ margin: '10px 0', color: '#475569' }}>{sub}</h4>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                                {getFiles(vert, cat, sub).length === 0 ? <small style={{color:'#94a3b8'}}>No files yet.</small> : 
                                                    getFiles(vert, cat, sub).map((f, idx, arr) => (
                                                    <div key={f._id} style={miniItemStyle}>
                                                        <span>{f.title} <small style={{color:'#64748b'}}>({f.resourceType} {f.board ? `- ${f.board}`:''})</small></span>
                                                        <div style={{display:'flex', gap:'5px', alignItems:'center'}}>
                                                            <button onClick={() => handleMove(f, -1, {vert, cat, sub})} style={{...arrowBtn, opacity: idx===0?0.3:1}} disabled={idx===0}>⬆️</button>
                                                            <button onClick={() => handleMove(f, 1, {vert, cat, sub})} style={{...arrowBtn, opacity: idx===arr.length-1?0.3:1}} disabled={idx===arr.length-1}>⬇️</button>
                                                            <button onClick={() => handleEdit(f._id, f.title)} style={miniEditBtn}>✎</button>
...
const sectionStyle = { background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '40px', border: '1px solid #e2e8f0' };
const miniItemStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', background: 'white', border: '1px solid #e2e8f0', borderRadius: '5px', fontSize: '0.9rem' };
...
const arrowBtn = { background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer', padding: '2px 6px', marginRight: '5px' };
